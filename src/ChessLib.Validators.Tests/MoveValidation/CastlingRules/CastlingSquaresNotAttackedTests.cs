...
namespace ChessLib.Validators.Tests.MoveValidation.CastlingRules
{
    ...
    public class CastlingSquaresNotAttacked : ChessLib.Validators.MoveValidation.CastlingRules.CastlingSquaresNotAttacked
    {
        ...
        public static void IsKingsPathInCheck_ShouldReturnFalse_IfKingUnaffectedByAnyPiece()
        {
            var move = MoveHelpers.GenerateMove(60, 62, MoveType.Castle);
            var position = new BoardFENInfo("4k2r/8/8/8/8/8/8/4K3 b kq - 1 2");
            Assert.IsFalse(IsKingsPathInCheck(position.OpponentColor(), position.PiecePlacement.GetPiecePlacementArray(), move), "IsKingsPathInCheck() should return false when nothing blocks castling privilege.");
        }
        ...
        {
            var move = MoveHelpers.GenerateMove(60, 62, MoveType.Castle);
            var pos1 = new BoardFENInfo("4k2r/8/8/8/8/8/8/4KR2 b kq - 1 2");
            var pos2 = new BoardFENInfo("4k2r/8/8/8/8/8/8/4K1R1 b kq - 1 2");
            var pos3 = new BoardFENInfo("4k2r/8/8/8/8/8/4R3/4K3 b kq - 1 2");

            Assert.IsTrue(IsKingsPathInCheck(pos1.OpponentColor(), pos1.PiecePlacement.GetPiecePlacementArray(), move), "IsKingsPathInCheck() should return true when Rook on f1 blocks castling privilege.");
            Assert.IsTrue(IsKingsPathInCheck(pos2.OpponentColor(), pos1.PiecePlacement.GetPiecePlacementArray(), move), "IsKingsPathInCheck() should return true when Rook on g1 blocks castling privilege.");
            Assert.IsTrue(IsKingsPathInCheck(pos3.OpponentColor(), pos1.PiecePlacement.GetPiecePlacementArray(), move), "IsKingsPathInCheck() should return true when Rook on e2 blocks castling privilege.");
            Assert.IsTrue(IsKingsPathInCheck(pos3.OpponentColor(), pos1.PiecePlacement.GetPiecePlacementArray(), move), "IsKingsPathInCheck() should return true when Rooks on f1-h1 block castling privilege.");
        ...
        public static void IsKingsPathInCheck_ShouldReturnFalse_IfKingUnaffectedByOpposingRook()
        {
            ...
            Assert.IsFalse(IsKingsPathInCheck(position.OpponentColor(), position.PiecePlacement.GetPiecePlacementArray(), move), "IsKingsPathInCheck() should return false when Rook on h1 doesn't block castling privilege.");
        }
