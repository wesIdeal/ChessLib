using ChessLib.Data.Boards;
using ChessLib.Data.Types.Enums;
using NUnit.Framework;

namespace ChessLib.Data.Tests.Boards
{
    [TestFixture]
    public class BoardStateTests
    {
        [TestCase(255, 47, Piece.Queen, CastlingAvailability.BlackKingside | CastlingAvailability.BlackQueenside |
                                        CastlingAvailability.WhiteKingside | CastlingAvailability.WhiteQueenside)]
        [TestCase(0, null, null, CastlingAvailability.NoCastlingAvailable)]
        [TestCase(21, null, Piece.Pawn, CastlingAvailability.WhiteKingside)]
        public void TestMax(int hm, int? ep, Piece? capPiece, CastlingAvailability ca)
        {
            var boardState = new BoardState((ushort) hm, (ushort?) ep, capPiece,  ca);
            Assert.AreEqual(hm, boardState.GetHalfmoveClock());
            Assert.AreEqual(ep, boardState.GetEnPassantSquare());
            Assert.AreEqual(capPiece, boardState.GetPieceCaptured());
            Assert.AreEqual(ca, boardState.GetCastlingAvailability());
        }
    }
}